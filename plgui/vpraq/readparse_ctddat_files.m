...
for j=1:sd,
    ...
    for k=1:sd2,
        ...
        %         sidx=find(diff(seconds)==-1)+1;%check if/where seconds go backwards and correct in following line
        %         for kk=1:length(sidx), seconds(sidx(kk))=(seconds(sidx(kk)+1)+seconds(sidx(kk)-1))/2;end
        for sj=1:size(seconds,1)-1;if seconds(sj)>seconds(sj+1),seconds(sj+1)=seconds(sj);end;end
        
        %find bad (still negative)values (<0)
        %         badvalues=find(diff(seconds)<0);
        
        si=find(diff(seconds));%seconds index - 1, i.e., diff=1 at index before new second
        
        %take only the positive / increasing values of time (monotonically non-decreasing)
        si=find(diff(seconds)>0);
